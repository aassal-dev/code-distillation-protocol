...
**Version:** 0.0.4 **Type:** Workflow / Standard Operating Procedure **Inputs:** Local Git Clone Path **Outputs:** Atomic Pattern Files in Global Library
...
**Prerequisite:** You MUST clone the repository locally. Do not use flat files (like uithub) as this breaks the Agent's ability to navigate the file system structure.

## Phase 1: The Survey (Single Thread)

**Agent:** Surveyor (High Context Window) **Goal:** Map the territory and define work packages.
...
**Agent:** Extractors (Multiple Instances) **Input:** One entry from `extraction_targets.json`.
...
**Agent:** Librarian (High Reasoning) **Input:** Folder containing all `raw_finding_*.md` files.
