...
**Version:** v0.2.06 (add-on to Knowledge Extraction workflow)
...
In v0.2.06, the Blueprint is the optional high-level input for Phase 0 (Director). It is **add-on**: the core pipeline (Director → Survey → Atomize → Librarian) stays one path; Blueprint is enabled when the user supplies a theory file path in config or context.
...
- **Director (step-00):** Reads Blueprint + README/docs; produces learning_objectives.md (Learning Targets proportional to code size, derived from or aligned with the Blueprint). Optionally produces implementation_map.json.
