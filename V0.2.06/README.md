# Knowledge Extraction — v0.2.06

This folder is the **v0.2.06** release of the Code Distillation Protocol (Knowledge Extraction). Use everything in this folder together; do not mix with files from other version folders (e.g. V0.2.05).
...
| **v0.2.06 Standardized Output Templates.md**            | Single template for findings and patterns (frontmatter + sections).                              |
| **v0.2.06 The Codebase Distillation Prompts.md**        | Prompt text for Director, Surveyor, Atomizer, Librarian.                                         |
| **v0.2.06 The Distillation Workflow (Orchestrator).md** | In-editor driver: copy into Composer to run the full workflow.                                   |
| **extraction-state.schema.md**                          | Schema for extraction-state.json (resume, optional merge_map_path).                              |
...
- **installed_path** must be the folder that contains this README (the V0.2.06 folder).

If you run from the **repo you are distilling**, set in workflow.yaml:

- **extraction_root** — repo to distill (often the same repo or a path like `E:\...\openclaw`).
- **installed_path** — path to this V0.2.06 folder so `validation` resolves to `V0.2.06/checklist.md`.
...
- `installed_path: "D:/wkspaces/Obsidian Vaults/.../V0.2.06"`  
  so that `validation: "{installed_path}/checklist.md` points at the checklist inside V0.2.06.

### 2. Run in Cursor (in-editor)

1. Open the **repository you want to distill** in Cursor (the codebase, not the protocol folder).
2. Open **v0.2.06 The Distillation Workflow (Orchestrator).md** from this V0.2.06 folder.
...
4. In the same message (or in config), tell the agent where the **V0.2.06 folder** is, e.g.:  
   _“Use the workflow and steps from: D:\wkspaces\Obsidian Vaults\...\V0.2.06”_
...
- **installed_path** — Path to **this** V0.2.06 folder (so validation uses this folder’s checklist).
...
- `_distillation/learning_objectives.md` (if Director runs)
- `_distillation/extraction_plan.json`
- `_distillation/findings/*.md`
- `_distillation/library/pattern_*.md`, `library/README.md`, `library/merge-map.json` (when merges occur)
...
This version adds: **Code completeness** (minimal slice or full example per finding/pattern; pointer exception), **merge-map** (library/merge-map.json), optional **concept_id**, **Learning Targets proportional to code size**, and **tagging/README** rules (prefer existing tags; every pattern in at least one quick-lookup bucket). For the previous behaviour, use the **V0.2.05** folder instead.
