# Standardized Output Templates (v0.2.06)
...
### YAML frontmatter (required + optional)
...
tags: [tag1, tag2, tag3]
# Optional (v0.2.06): code_completeness: minimal | full | pointer
# Optional (v0.2.06): concept_id: short-kebab-concept (e.g. hitl-async-suspension)
---
```
...
- **tags:** Semantic tags for retrieval (e.g. regex, template-engine, cli, security). **Tagging guideline:** Prefer existing tags from the library README semantic tag table; add new tags only when no existing tag fits, and consider adding the new tag to the README table in the same run.
- **code_completeness** _(optional):_ `minimal` = 5–15 line working slice; `full` = complete small example (one function/schema block); `pointer` = implementation requires source repo—Code may be comments/references only. Atomizer sets it when writing findings; Librarian preserves or upgrades when promoting to pattern.
- **concept_id** _(optional):_ Short kebab-case concept name (e.g. `hitl-async-suspension`) for merge traceability; can be set in extraction plan and copied through to pattern.
...
4. **## Code** — One or more code blocks showing the implementation. **Code completeness (v0.2.06):** The Code section must contain at least one of: (a) a **minimal working slice** (typically 5–15 lines) showing core logic, data flow, or API, or (b) a **complete small example** (e.g. one function, one schema block) that is copy-paste adaptable. **Exception:** Code may be comment-only or "see source_path" only if the finding is explicitly marked as **pointer-type** (`code_completeness: pointer`); document that in the template. For line-level findings, 1–5 lines of real code plus brief "how" and "why" (e.g. "this regex order matters because …") are acceptable.
...
**Reference:** Align with distillation pattern format in BMAD-METHOD _distillation/patterns (pattern_\*.md and README.md). Do not introduce a different schema for findings vs library; this template is the single source of truth. v0.2.06: code_completeness, concept_id, Code completeness rules, tagging guideline.
