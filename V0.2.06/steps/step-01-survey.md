...
4. **For each** such area, add one JSON entry with:
   ...
   - `concept_id` _(optional):_ short kebab-case concept name (e.g. `hitl-async-suspension`) when the target clearly maps to a concept for merge traceability.
...
[
  {
    ...
    "concept_id": "optional-short-kebab-concept"
  }
]
```

Required: `target_id`, `path`, `focus`, `abstraction_level`. Optional (v0.2.06): `concept_id` (short kebab-case for merge traceability; Surveyor may set when learning_objectives or focus implies a concept). Do not use `id`, `depth`, or other field names.
