...
- ✅ **Learning Targets:** Produce 3–7 specific Learning Targets (questions or goals). Each target should be a concrete architectural or algorithmic question the Surveyor will answer by mapping code (e.g. "How does the Context Engine manage token limits during graph traversal?").
...
Before any code survey, define _what_ we are hunting for. Read README and docs (and optional Blueprint); produce a short learning_objectives.md with 3–7 Learning Targets so the Surveyor can map code to those targets.
...
4. **Formulate** 3–7 Learning Targets: specific questions or goals (e.g. "How is X implemented?", "Where does Y handle Z?").
...
- **Primary:** `learning_objectives.md` at output_objectives. Content: 3–7 Learning Targets (specific questions or goals).
