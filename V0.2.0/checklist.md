# Knowledge Extraction — Validation Checklist (v0.2.0)
...
- [ ] **Sections:** Each finding follows the single template: Problem, Solution, Code, AI Rule.
...
- [ ] **Sections:** Each pattern uses the ordered pipeline: frontmatter → Problem → Solution → Code → AI Rule.
...
- [ ] **State file:** If using resume, extraction-state.json is updated (current_phase, last_updated, and optionally targets_done, findings_count, discarded_common_knowledge_count).
